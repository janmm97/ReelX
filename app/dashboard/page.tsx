...
const MODELS: {
  ...
  // ── Standard ──────────────────────────────────────────────────
  {
    ...
    color: 'from-violet-500 to-slate-300',
    tier: 'standard',
    cost: '$14.65/M',
  },
  {
    ...
    color: 'from-white to-violet-600',
    tier: 'standard',
    cost: '$17.09/M',
  },
  {
...
const VIDEO_MODELS: VideoModelDef[] = [
  ...
  },
  {
    ...
    color: 'from-blue-600 to-slate-300',
    tier: 'premium',
  },
  {
    id: 'kling3_audio',
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {ONBOARDING_STEPS.map((_, i) => (
            <div key={i} style={{
              ...
              background: i <= step ? '#FFFFFF' : '#1C1C1C',
              transition: 'background 0.3s',
            }} />
          ))}
        </div>
        <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 22, fontWeight: 700, color: '#FFFFFF', margin: 0 }}>
          {current.heading}
        </h2>
            ...
            {(current.options as readonly string[]).map(opt => {
              ...
              return (
                <button key={opt} onClick={() => onSelect(opt)} style={{
                  background: selected ? 'rgba(255,255,255,0.06)' : '#101010',
                  border: selected ? '1px solid rgba(255,255,255,0.4)' : '1px solid #1C1C1C',
                  borderRadius: 12, padding: '14px 16px', textAlign: 'left',
                  color: selected ? '#FFFFFF' : '#999999', fontSize: 14, cursor: 'pointer',
...
function DashboardInner() {
  ...
  return (
    <div className="flex flex-col h-screen text-white overflow-hidden" style={{ background: '#000000' }}>
      ...
      <header style={{
        ...
        padding: '0 20px', background: '#101010', borderBottom: '1px solid #1C1C1C',
        flexShrink: 0, zIndex: 40,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Image src="/brand/reelx-icon-white-svg.svg" alt="Reelx" width={26} height={26} style={{ objectFit: 'contain' }} />
        </div>
        {/* Search bar */}
        <div style={{
          ...
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
            <circle cx="11" cy="11" r="7" stroke="#787878" strokeWidth="2"/>
            <path d="M16.5 16.5L21 21" stroke="#787878" strokeWidth="2" strokeLinecap="round"/>
          </svg>
          <input
            placeholder="Search your creations…"
            style={{ background: 'none', border: 'none', outline: 'none', color: '#FFFFFF', fontSize: 13, width: '100%' }}
          />
        </div>
        {/* Right controls */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {user && (
            <>
              {user.avatar_url ? (
                ...
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: '#161616', border: '1px solid #1C1C1C', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: '#787878' }}>
                  {(user.name ?? user.email)[0].toUpperCase()}
                </div>
              )}
              <button
                onClick={handleSignOut}
                style={{ fontSize: 12, color: '#787878', background: 'none', border: 'none', cursor: 'pointer' }}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Sidebar nav ── */}
        <nav style={{
          width: sidebarCollapsed ? 52 : 192, background: '#101010', borderRight: '1px solid #1C1C1C',
          ...
          <div style={{ display: 'flex', justifyContent: sidebarCollapsed ? 'center' : 'flex-end', padding: '10px 10px 4px' }}>
            <button
              ...
              style={{
                ...
                cursor: 'pointer', color: '#787878', flexShrink: 0,
              }}
              onMouseEnter={e => (e.currentTarget.style.color = '#FFFFFF')}
              onMouseLeave={e => (e.currentTarget.style.color = '#787878')}
            >
          ...
          {!sidebarCollapsed && (
            <div style={{ padding: '16px 16px', borderTop: '1px solid #1C1C1C' }}>
        ...
        {/* ── Left Panel ── */}
        <aside className="w-[340px] shrink-0 flex flex-col border-r overflow-y-auto" style={{ borderColor: '#1C1C1C', background: '#101010' }}>
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className={`transition-colors p-0.5 rounded ${enhancingPrompt ? 'text-white animate-pulse' : 'text-slate-600 hover:text-white'} disabled:opacity-40 disabled:cursor-not-allowed`}
                >
                  {enhancingPrompt ? (
          ...
          {promptExpanded && (
            ...
            >
              <div
                className="relative bg-[#161616] border border-white/10 rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col"
              ...
              >
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-medium border transition-all
                        ${enhancingPrompt
                          ...
                          : 'border-white/10 text-slate-400 hover:border-white/25 hover:text-white hover:bg-white/5'
                ...
                <div className="p-5 flex-1 overflow-hidden flex flex-col">
                  <textarea
                    ...
                    disabled={loading}
                    className="flex-1 w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 resize-none focus:outline-none focus:ring-1 focus:ring-white/20 disabled:opacity-50 leading-relaxed"
                    style={{ minHeight: 280 }}
                    autoFocus
                  />
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <button
                      onClick={() => setPromptExpanded(false)}
                      className="px-4 py-2 rounded-lg text-sm font-medium bg-white text-black hover:opacity-90 transition-opacity"
                    >
                      Done
                    </button>
                  </div>
                ...
                <PanelRow icon={<ModelIcon />} label="AI Model">
                  <div className="relative">
                    <select
                      ...
                      disabled={loading}
                      className="appearance-none bg-white/[0.06] border border-white/[0.08] rounded-lg pl-3 pr-7 py-1.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-white/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer min-w-[140px] max-w-[170px]"
                    >
                      {imageMode === 'blend' ? (
                  ...
                  <div className="mb-1">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      {blendImages.length < 8 && (
                        <label className="shrink-0 w-20 h-20 rounded-lg border-2 border-dashed border-white/[0.1] hover:border-white/25 transition-colors flex flex-col items-center justify-center gap-1 cursor-pointer">
                          <UploadIcon />
                      ...
                      {uploadedImages.map((img, i) => (
                        <div key={i} className="shrink-0 flex flex-col gap-1">
                          ...
                          <input
                            ...
                            className="w-20 bg-white/[0.05] border border-white/[0.08] rounded px-1.5 py-1 text-[10px] text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-white/20"
                          />
                        </div>
                      ))}
                      {/* Add image tile */}
          ...
          <div className="px-5 pb-5 pt-2 shrink-0">
            <button
              ...
              className="w-full py-3.5 rounded-xl font-bold text-sm bg-white text-black font-bold disabled:opacity-40 disabled:cursor-not-allowed transition-opacity hover:opacity-88 active:scale-[0.98] flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
                  <Spinner />
            ...
            {loading ? (
              <div className="flex flex-col items-center gap-4">
                <div className="relative w-24 h-24">
                  ...
                  <div className="absolute inset-2 rounded-full border-2 border-white/40 animate-spin" style={{ animationDuration: '2s' }} />
                  <div className="absolute inset-4 rounded-full border-2 border-white/25 animate-spin" style={{ animationDuration: '1.5s', animationDirection: 'reverse' }} />
              ...
              <div className="h-full flex flex-col items-center gap-3 max-w-2xl w-full">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#161616]/15 border border-white/20 text-white/70 hover:bg-white/5 hover:text-white transition-all text-xs font-medium"
                >
          ...
          <div className="shrink-0 border-t border-white/[0.07] p-4">
            ...
            {historyLoading ? (
              ...
              <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-none">
                {history.map((item) => (
                  <div key={item.id} className="group shrink-0 w-28 relative rounded-xl overflow-hidden ring-1 ring-white/10 hover:ring-white/20 hover:scale-105 transition-all duration-200">
      ...
      {expanded && (
        ...
        >
          <div
            className="relative bg-[#161616] border border-white/10 rounded-2xl overflow-hidden max-w-2xl w-full shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            {expanded.kind === 'image' ? (
              /* eslint-disable-next-line @next/next/no-img-element */
